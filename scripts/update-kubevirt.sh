#!/bin/bash
#
# Update KubeVirt types from upstream swagger spec
#
# Usage: ./scripts/update-kubevirt.sh [version]
#   version: Branch or tag name (default: main)
#
# Example:
#   ./scripts/update-kubevirt.sh           # Uses main branch
...
TARGET_DIR="$PROJECT_ROOT/src/generated/kubevirt"

# Upstream swagger URL
SWAGGER_URL="https://raw.githubusercontent.com/kubevirt/kubevirt/${VERSION}/api/openapi-spec/swagger.json"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Updating KubeVirt Types${NC}"
...
# Done
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}KubeVirt types updated successfully!${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "${RED}IMPORTANT: You must check for export conflicts!${NC}"
echo ""
echo -e "${YELLOW}Next steps:${NC}"
echo -e "  1. Run './scripts/check-conflicts.sh' to detect conflicts"
