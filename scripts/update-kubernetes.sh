#!/bin/bash
#
# Update Kubernetes types from upstream swagger spec
#
# Usage: ./scripts/update-kubernetes.sh [version]
#   version: Branch or tag name (default: master)
#
# Example:
#   ./scripts/update-kubernetes.sh           # Uses master branch
...
TARGET_DIR="$PROJECT_ROOT/src/generated/kubernetes"

# Upstream swagger URL
SWAGGER_URL="https://raw.githubusercontent.com/kubernetes/kubernetes/${VERSION}/api/openapi-spec/swagger.json"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Updating Kubernetes Types${NC}"
...
# Done
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Kubernetes types updated successfully!${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "${YELLOW}Next steps:${NC}"
echo -e "  1. Run 'npm run build' to verify no errors"
echo -e "  2. Check for any new export conflicts with KubeVirt/CDI"
echo -e "  3. Update the Version Tracking table in MAINTENANCE.md"
echo ""
