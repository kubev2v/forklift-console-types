/**
 * 
 * 
 *
 * The version of the OpenAPI document: 
 * Contact Email: 
 * License: 
 *
 * NOTE: This file is auto generated by crdtotypes (https://github.com/yaacov/crdtoapi/).
 * https://github.com/yaacov/crdtoapi/README.crdtotypes
 */

import { V1beta1PlanStatusMigrationVmsPipelineError } from './V1beta1PlanStatusMigrationVmsPipelineError';
import { V1beta1PlanStatusMigrationVmsPipelineProgress } from './V1beta1PlanStatusMigrationVmsPipelineProgress';
import { V1beta1PlanStatusMigrationVmsPipelineTasks } from './V1beta1PlanStatusMigrationVmsPipelineTasks';
import { V1beta1PlanStatusMigrationVmsPipelineWarning } from './V1beta1PlanStatusMigrationVmsPipelineWarning';

/**
 * Pipeline step.
 *
 * @export
 */
export interface V1beta1PlanStatusMigrationVmsPipeline {
  /** annotations
   * Annotations.
   *
   * @required {false}
   * @originalType {V1beta1PlanStatusMigrationVmsPipelineAnnotations}
   */
  annotations?: {[key: string]: string};
  /** completed
   * Completed timestamp.
   *
   * @required {false}
   * @format {date-time}
   */
  completed?: string;
  /** description
   * Name
   *
   * @required {false}
   */
  description?: string;
  /** error
   * Error.
   *
   * @required {false}
   */
  error?: V1beta1PlanStatusMigrationVmsPipelineError;
  /** name
   * Name.
   *
   * @required {true}
   */
  name: string;
  /** phase
   * Phase
   *
   * @required {false}
   */
  phase?: string;
  /** progress
   * Progress.
   *
   * @required {true}
   */
  progress: V1beta1PlanStatusMigrationVmsPipelineProgress;
  /** reason
   * Reason
   *
   * @required {false}
   */
  reason?: string;
  /** started
   * Started timestamp.
   *
   * @required {false}
   * @format {date-time}
   */
  started?: string;
  /** tasks
   * Migration task.
   *
   * @required {false}
   */
  tasks?: V1beta1PlanStatusMigrationVmsPipelineTasks[];
  /** warning
   * Warning.
   *
   * @required {false}
   */
  warning?: V1beta1PlanStatusMigrationVmsPipelineWarning;
}
